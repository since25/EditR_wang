...
getEditing <- function(ab1file, exampleguide, outputF = NA, output = list(), trim3 = NA, trim5 = NA) {
  ...
  input.seqReactive <- function() { readsangerseq(ab1file) }
  input.basecallsReactive <- function() { makeBaseCalls(input.seqReactive()) }
  guideReactive <- function() { DNAString(exampleguide) }
  p.val.Reactive <- function() { 0.01 }
  
  sangsReactive <- function() {
  ...
  sangs.filtReactive <- function() {
    sangs <- sangsReactive()
    if (is.na(trim3) & is.na(trim5)) {
  ...
  nullparams.Reactive <- function() {
    sangs.filt <- sangs.filtReactive()
    guide.coord <- guide.coordReactive()
    null.m.params <- GetNullDistModel(sangs.filt, guide.coord)
  }
  
  editing.Reactive <- function() {
    sangs.filt <- sangs.filtReactive()
    sangs <- sangsReactive()
    guide.coord <- guide.coordReactive()
    guide <- guideReactive()
    invisible(capture.output(null.m.params <- nullparams.Reactive(), type = "output"))
    editing.df <- CreateEditingDF(guide.coord, guide, sangs, null.m.params)
    return(editing.df)
  }
  
  write.csv(editing.Reactive(), outputF)
  cat("输出结果至 ", outputF, "。\n")
  cat("结束分析", ab1file, "。\n")
  cat("=========================================================================\n\n")
}
...
# 定义 extract_cell 函数
extract_cell <- function(csv_file, row_index, column_index) {
  data <- read.csv(csv_file, header = TRUE)
  cell_content <- NA
  if (row_index <= nrow(data) && column_index <= ncol(data)) {
    cell_content <- data[row_index, column_index]
  }
  return(cell_content)
}
